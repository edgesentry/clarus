...
impl FileReplayAdapter {
    ...
    pub fn from_csv(content: &str) -> Result<Self, String> {
        ...
        let mut frames: std::collections::BTreeMap<u64, Vec<Entity>> =
        ...
        for (i, line) in lines.enumerate() {
            ...
            frames.entry(ts).or_default().push(Entity {
                id,
                class,
    ...
    /// Return the next frame, or `None` when all frames have been replayed.
